  ...
  $params['current_year'] = date('Y'); // เช่น 2025
   ...
   // เพิ่มข้อมูลห้องสอบ (room)
    ...
    public function enter_your_score($request)
    {
        ...
        // ตรวจสอบอินพุต
        if (is_null($application_id) || is_null($written_score) || is_null($interview_score) || is_null($final_score)) {
            return false;
        }
...
public function create_room($request)
{
    ...
    $announcement_date = $request->input('announcement_date'); // ควรเป็น YYYY-MM-DD
    $exam_location   = $request->input('exam_location');
    $schedule        = $request->input('schedule');
    $details         = $request->input('details');
    $announce         = $request->input('announce');
    $uploader = new uploadDocumentFile();
    $uploadResult = $uploader->uploadDocumentFile($request);
        // ถ้า uploadResult เป็น response (error) ให้ return ทันที
    if ($uploadResult instanceof \Illuminate\Http\JsonResponse) {
        return $uploadResult;
    }
    $document_url = $uploadResult;
    // ตรวจสอบว่ามี job_id นี้อยู่ใน exam_announcements หรือยัง
    $existing = DB::table('exam_announcements')
                ->where('job_id', $job_id)
                ->first();

    if ($existing) {
        // ถ้ามี -> อัพเดตข้อมูล
        DB::table('exam_announcements')
            ->where('job_id', $job_id)
            ->update([
                'title'            => $title,
        ...
        // ถ้าไม่มี -> เพิ่มใหม่
        DB::table('exam_announcements')->insert([
            'job_id'            => $job_id,
            'title'             => $title,
            'announcement_date' => $announcement_date,
            'exam_location'     => $exam_location,
